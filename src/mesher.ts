...
export function resolveFace(id: BlockId, faceIndex: number, liquidDistance: number = 0, data?: BlockData): FaceResolution {
  ...
  if (id === BlockId.MESSAGE_BLOCK) return { material: MATERIAL_OPAQUE, atlasKey: 'message' };
  if (id === BlockId.HOLOGRAM_BLOCK) return { material: MATERIAL_OPAQUE, atlasKey: 'hologram' };
