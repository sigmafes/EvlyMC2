...
// Copiado de src/water-engine.ts - ver world-server/src/game/README.md para el
// criterio de qué se copia vs qué sigue compartido con singleplayer.
// Cambios acá NO se reflejan automáticamente en src/water-engine.ts.
//
// Qué se sacó: `getWaterFlow()` y sus ayudantes (`renderedDataAt`,
// `isFallingAt`), que devuelven un THREE.Vector3 y solo sirven para empujar al
// jugador y animar la superficie del lado cliente. La simulación en sí queda
// idéntica: mismas distancias, mismo tickInterval, misma regla de fuente
// infinita, mismo getSpread/getSlopeDistance de MCPE.
//
// La única diferencia real: donde singleplayer preguntaba
// `withinFluidSimRadius(x, z, playerX, playerZ)` - un radio alrededor del
// único jugador - acá se recibe un predicado `isActiveAt` que es la unión de
// los alrededores de TODOS los jugadores conectados (game/active-region.ts).
// El comportamiento buscado es el mismo: lo que queda fuera se congela tal
// cual está, sin drenarse ni avanzar, y retoma solo cuando alguien vuelve.
...
export class WaterEngine {
  ...
  constructor(
    private readonly world: FluidWorld,
    config: LiquidConfig = {
  ...
  update(delta: number, isActiveAt: IsActiveAt): boolean {
    this.timeSinceLastTick += delta;
    if (this.timeSinceLastTick < this.tickInterval) return false;
    this.timeSinceLastTick = 0;
    return this.tick(isActiveAt);
  }

  private tick(isActiveAt: IsActiveAt): boolean {
    ...
    const target = this.computeTargetWater(isActiveAt);
    let changed = false;
    ...
    for (const [key, node] of this.currentWater) {
      if (!target.has(key) && !isActiveAt(node.x, node.z)) target.set(key, node);
    }
  ...
  private computeTargetWater(isActiveAt: IsActiveAt): Map<string, WaterNode> {
    ...
    for (const s of this.sources.values()) {
      ...
      // being in target".
      if (!isActiveAt(s.x, s.z)) continue;
