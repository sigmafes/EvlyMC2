// Copiado de src/fire-engine.ts - ver world-server/src/game/README.md para el
// criterio de qué se copia vs qué sigue compartido con singleplayer.
// Cambios acá NO se reflejan automáticamente en src/fire-engine.ts.
//
// La simulación es idéntica: mismas probabilidades de propagación y consumo
// (FireTile::tick de LCE), mismo envejecimiento, misma ignición desde lava.
// Solo cambian dos cosas:
//
// 1. El radio de un único jugador (`withinFluidSimRadius`) pasa a ser el
//    predicado `isActiveAt`, que es la unión de los alrededores de todos los
//    jugadores conectados (game/active-region.ts). Mismo comportamiento: lo
//    que queda fuera se congela ardiendo tal cual está, sin envejecer ni
//    propagarse, y retoma cuando alguien vuelve a acercarse.
// 2. Hay un tope de celdas de fuego simultáneas (MAX_FIRE_CELLS). En
//    singleplayer el incendio está acotado por lo que un jugador puede
//    prender; acá varios jugadores dispersos pueden tener incendios grandes a
//    la vez, y el trabajo por tick crece con el total. Es una red de
//    seguridad para la CPU del Durable Object, no una regla de juego.

import { BlockId, blockLightProperties, isFlammable } from '../../../src/block';
import { CHUNK_HEIGHT } from '../../../src/chunk';
import type { FluidWorld, IsActiveAt } from './fluid-world';
import type { WaterEngine } from './water-engine';

/** Safety net on simultaneous burning cells - see the header note. Past this, existing fire keeps burning out but no new cell catches. */
const MAX_FIRE_CELLS = 512;
...
export class FireEngine {
  ...
  /** Re-register a FIRE block that already exists in the world but that this engine has no record of - see world-do.ts's restore on wake-up. Without it those blocks would burn forever, since only tracked cells ever age out. */
  adopt(x: number, y: number, z: number): void {
  ...
  update(delta: number, isActiveAt: IsActiveAt, lava?: WaterEngine): boolean {
    this.timeSinceTick += delta;
    if (this.timeSinceTick < this.tickInterval) return false;
    this.timeSinceTick = 0;
    return this.tick(isActiveAt, lava);
  }

  private tick(isActiveAt: IsActiveAt, lava?: WaterEngine): boolean {
    let changed = false;

    // --- Lava ignites nearby flammable blocks (LCE LiquidTileStatic::tick). ---
    if (lava) {
      for (const c of lava.cells()) {
        if (!isActiveAt(c.x, c.z)) continue;
  ...
  private setFire(x: number, y: number, z: number, age: number) {
    ...
    const k = key(x, y, z);
    // Past the cap, existing fire still ages and burns out normally - only
    // NEW cells are refused, so a runaway blaze plateaus and then recedes
    // instead of growing without bound. Re-igniting a cell already tracked is
    // always allowed, since it doesn't grow the set.
    if (this.fires.size >= MAX_FIRE_CELLS && !this.fires.has(k)) return;
    this.world.setBlock(x, y, z, BlockId.FIRE);
    this.fires.set(k, Math.max(0, Math.min(15, age)));
  }
