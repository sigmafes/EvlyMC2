// Copiado de src/mob-ai.ts el 2026-09-14 - ver world-server/src/game/README.md
// para el criterio de qué se copia vs qué sigue compartido con singleplayer.
// Cambios acá NO se reflejan automáticamente en src/mob-ai.ts, y viceversa.
//
// ARROW_GRAVITY/powerToSpeed están inlineados abajo en vez de importados de
// arrow-projectiles.ts - ese archivo arrastra block-preview/item-stack/
// inventory (mallas 3D, texturas), nada de lo cual hace falta para estas
// dos constantes puramente numéricas. Si cambia el "feel" del tiro del
// arquero en singleplayer, este valor hay que actualizarlo a mano acá.
...
// De src/arrow-projectiles.ts - mantener en sync a mano con esas dos constantes.
const ARROW_GRAVITY = 20;
const POWER_TO_SPEED = 15;
function powerToSpeed(power: number): number {
